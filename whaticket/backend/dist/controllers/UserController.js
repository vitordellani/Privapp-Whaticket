...
exports.store = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { email, password, name, profile, queueIds, whatsappId, whatsappNotificationNumber, notifyOnNewMessage } = req.body;
    if (req.url === "/signup" &&
...
exports.update = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (req.user.profile !== "admin") {
        // Allow self-update for notification preferences only
        if (String(req.user.id) !== String(userId)) {
            throw new AppError_1.default("ERR_NO_PERMISSION", 403);
        }
        const { whatsappNotificationNumber, notifyOnNewMessage } = req.body;
        const limitedData = { whatsappNotificationNumber, notifyOnNewMessage };
        user = yield UpdateUserService_1.default({ userData: limitedData, userId });
    }
    else {
        const userData = req.body;
        user = yield UpdateUserService_1.default({ userData, userId });
    }
    const io = socket_1.getIO();
    io.emit("user", {
        action: "update",
