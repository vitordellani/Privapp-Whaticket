...
async function ZDGSender(number, url, title, iD, token, isGroup) {

  const urlPost = process.env.REACT_APP_BACKEND_URL + '/sendMedia';
  const headers = {
...
const ZDGMedia = () => {
	  ...
	  const handleSubmit = async (event) => {
		...
		if (!checked){
			for (const user of usersTextArea){
				...
				if (numberDDI !== "55") {
					ZDGSender(user, inputs.url, inputs.title, inputs.id, token, checked);
					await timer(rndInt * 1000)				
					...
					await timer(rndInt * 1000)
					ZDGSender(numberDDI.toString() + numberDDD.toString() + "9" + numberUser.toString(), inputs.url, inputs.title, inputs.id, token, checked);
					setMessageCount(prevCount => prevCount + 1);
				}
				else if (numberDDI === "55" && parseInt(numberDDD) > 30) {
					const numberUser = user.substr(-8,8);
					await timer(rndInt * 1000)
					ZDGSender(numberDDI.toString() + numberDDD.toString() + numberUser.toString(), inputs.url, inputs.title, inputs.id, token, checked);				
					setMessageCount(prevCount => prevCount + 1);
				}
			}
		}
	...
	const handleGroups= (event) => {
		...
		if(!checked)
		{
		  alert('Disparo para Grupos ativado.')
		}
		else if (checked){
		  alert('Disparo para Grupos desligado.')
		}
	};
