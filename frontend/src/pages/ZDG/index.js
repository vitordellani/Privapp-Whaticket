...
async function ZDGSender(number, message, iD, token, isGroup) {
	...
	const handleSubmit = async (event) => {
		...
		if (!checked){
			for (const user of usersTextArea){
				...
				if (numberDDI !== "55") {
					ZDGSender(user, inputs.message, inputs.id, token, checked);
					await timer(rndInt * 1000)
					...
					await timer(rndInt * 1000)
					ZDGSender(numberDDI.toString() + numberDDD.toString() + "9" + numberUser.toString(), inputs.message, inputs.id, token, checked);
					setMessageCount(prevCount => prevCount + 1);
				}
				else if (numberDDI === "55" && parseInt(numberDDD) > 30) {
					const numberUser = user.substr(-8,8);
					await timer(rndInt * 1000)
					ZDGSender(numberDDI.toString() + numberDDD.toString() + numberUser.toString(), inputs.message, inputs.id, token, checked);
					setMessageCount(prevCount => prevCount + 1);
				}
			}
		}
